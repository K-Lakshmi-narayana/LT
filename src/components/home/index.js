...
class Home extends Component {
    state = { "testDetails": [], "track_details": [], "isLoading": false, "tier": 1}
    ...
    renderTests = () => {
        const { testDetails, tier } = this.state
        const elements = []
        for (let i = 0; i < testDetails.length; i++) {
            const testObj = testDetails[i];
            testObj.type === "mcq" ? elements.push(
                <Card com={"home"} tier={tier} image={mcq} type={testObj.type} heading={testObj.name} desc={testObj.description} unqId={testObj.id} key={testObj.id} />
            ) : elements.push(
                <Card com={"home"} tier={tier} image={coding} type={testObj.type} heading={testObj.name} desc={testObj.description} unqId={testObj.id} key={testObj.id} />
            )
        }
        return elements
    }
            ...
            isLoading ? this.renderLoader() :
                <>
                    ...
                    {testDetails.length > 0 && <div className="tracks-container">
                        <div className="tests-head-con">
                            <h1 className="obj-heading">Competitions [Live]</h1>
                            {tier < 3 && <a href="/all-tests" className="obj-heading">All Tests{"..>"}</a>}
                        </div>
                    ...
                    {track_details.length > 0 &&
                        <div className="tracks-container">
                            <div className="tests-head-con">
                                ...
                                {testDetails.length === 0 && tier < 3 && <a href="/all-tests" className="obj-heading">All Tests{"..>"}</a>}
                            </div>
                            <div className="d-flex cards-container">
                                {track_details.map(eachTrack => {
                                    ...
                                    return <Card com={"home"} image={imageUrl} type="tracks" tier={tier} unqId={eachTrack.id} heading={eachTrack.title} desc={eachTrack.description} key={eachTrack.id} />
                                })}
                            </div>
                        </div>
                    }
